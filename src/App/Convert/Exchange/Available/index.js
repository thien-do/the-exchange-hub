...
const ConvertExchangeAvailable = ({ frm, to, exchange, markets, action }) => {
  // for "rate" and "fee"
  const detail = markets[frm.currency][to.currency]
    .find(exc => exc.name === exchange);
  ...
  return (
    <Container type="button" onClick={action}>
      <Title>
        <TitleHiglight>At: <Exchange value={exchange} /> </TitleHiglight>
...
export default ConvertExchangeAvailable;
